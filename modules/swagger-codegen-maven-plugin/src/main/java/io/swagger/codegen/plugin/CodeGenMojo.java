...
public class CodeGenMojo extends AbstractMojo {
	...
	private Collection<File> findSwaggerFiles(File folder) throws MojoExecutionException {
		if (!folder.exists()) {
			throw new MojoExecutionException(folder + " does not exist");
		}
		if (folder.exists() && !folder.isDirectory()) {
			throw new MojoExecutionException(folder + " is not a folder");
		}

		File[] filesAndFolders = folder.listFiles(new FileFilter() {
			@Override
			public boolean accept(File file) {
				return file.isDirectory() || (file.isFile() && file.getName().toLowerCase().endsWith(".yaml"));
			}
		});
		...
		for (File fileOrFolder : filesAndFolders) {
			if (fileOrFolder.isFile())
				...
				files.addAll(findSwaggerFiles(fileOrFolder));
			else
				throw new MojoExecutionException("Unknown file type: " + fileOrFolder);
		}
		return files;
	}

	private void handle(CodegenConfigurator configurator) throws MojoExecutionException {
		...
		if (configOptions != null) {
			for (CliOption langCliOption : config.cliOptions()) {
				if (configOptions.containsKey(langCliOption.getOpt())) {
					input.getConfig().additionalProperties().put(
						langCliOption.getOpt(),
